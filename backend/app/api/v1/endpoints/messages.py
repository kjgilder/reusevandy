...
async def get_conversations(
    ...
    for conv in conversations:
        ...
        if search:
            ...
            listing_match = search_lower in (
                conv.listing.title.lower() if conv.listing else ""
            )

            # Name match (check the OTHER person in the conversation)
            other_user = (
                conv.seller
                if str(current_user.id) == str(conv.buyer.id)
                else conv.buyer
            )
            name_match = (
                search_lower
                in (other_user.full_name.lower() if other_user.full_name else "")
...
async def get_pending_offers(current_user: User = Depends(deps.get_current_user)):
    ...
    for msg in messages:
        ...
        if not hasattr(msg.conversation, "listing") or not isinstance(
            msg.conversation.listing, Listing
        ):
            continue
...
async def get_conversation_details(
    conversation_id: UUID, current_user: User = Depends(deps.get_current_user)
):
    ...
    if (
        not isinstance(conversation.listing, Listing)
        or not isinstance(conversation.buyer, User)
        or not isinstance(conversation.seller, User)
    ):
        raise HTTPException(status_code=404, detail="Incomplete conversation data")

    if str(current_user.id) not in [
        str(conversation.buyer.id),
        str(conversation.seller.id),
    ]:
        raise HTTPException(
    ...
    messages = (
        await Message.find(Message.conversation.id == conversation.id)
        .sort("created_at")
        .to_list()
    )
    valid_messages = []
...
async def create_offer(
    offer_in: OfferCreate, current_user: User = Depends(deps.get_current_user)
):
    ...
    if str(listing.seller.id) == str(current_user.id):
        raise HTTPException(
            status_code=400, detail="You cannot make an offer on your own listing"
        )
...
async def initiate_conversation(
    init_in: InitiateConversation, current_user: User = Depends(deps.get_current_user)
):
...
async def send_message(
    ...
    if str(current_user.id) not in [
        ...
        raise HTTPException(
            status_code=403, detail="Not authorized to message in this conversation"
        )
    ...
    recipient = (
        conversation.seller
        if str(current_user.id) == str(conversation.buyer.id)
...
async def update_offer_status(
    ...
    if status_update.status not in ["accepted", "declined"]:
        raise HTTPException(
            status_code=400, detail="Invalid status. Must be 'accepted' or 'declined'"
        )
    ...
    if str(current_user.id) != str(message.conversation.seller.id):
        raise HTTPException(
            status_code=403, detail="Only the seller can accept or decline this offer"
        )
...
async def update_offer_amount(
    ...
    if str(current_user.id) != str(message.sender.id):
        raise HTTPException(
            status_code=403, detail="Only the sender can update this offer"
        )
